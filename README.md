...
* **TMC5160 SPI 模式 (推荐配置) 🚀**
    * **硬件设置**：使用跳线帽闭合 SPI 专用的针脚组，将 ESP32 的 SPI 总线（CS、SCK、MOSI、MISO）直接路由至 TMC5160 驱动器。
    * **系统优势**：彻底告别手动调节电位器！允许 OnStep 固件直接读取和写入寄存器。您可以在 Web 界面或 `Config.h` 中实时配置运行/保持电流、精确设置细分数（Microstepping），并在 StealthChop（极致静音）和 SpreadCycle（高动态扭矩）模式之间智能切换，最大化释放 25-100 谐波减速器的性能。
...
请参阅 [Firmware_Setup.md](Firmware_Setup.md) 获取完整的 `Config.h` 参数表及详细引脚映射。
